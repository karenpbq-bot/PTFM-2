...
def cargar_datos_supabase():
    """Conecta con Supabase, une bitacoras_taller y bitacoras_lineas, y normaliza la estructura."""
    try:
        supabase = conectar()
        
        # 1. Obtenemos las líneas de producción
        res_lineas = supabase.table("bitacoras_lineas").select(
            "proceso_bloque, cantidad, fecha_inicio, tipo_tablero_retazo, bitacora_id"
        ).execute()
        
        if not res_lineas.data:
            return pd.DataFrame()
            
        df_l = pd.DataFrame(res_lineas.data)
        
        # 2. Obtenemos el taller para traer fechas de respaldo (fecha de cabecera)
        res_taller = supabase.table("bitacoras_taller").select("id, fecha").execute()
        df_t = pd.DataFrame(res_taller.data) if res_taller.data else pd.DataFrame()
        
        if not df_t.empty and "id" in df_t.columns:
            df_t = df_t.rename(columns={"id": "bitacora_id", "fecha": "fecha_taller"})
            df = pd.merge(df_l, df_t, on="bitacora_id", how="left")
        else:
            df = df_l
            df["fecha_taller"] = None

        # Si fecha_inicio está vacía, usamos la fecha del taller como respaldo
        df["Fecha_Corte"] = df["fecha_inicio"].fillna(df["fecha_taller"])
        
        if df.empty or "Fecha_Corte" not in df.columns:
            return pd.DataFrame()

        # 3. Mapeo de procesos a las máquinas equivalentes del sistema anterior:
        # SECCIONADORA -> 'S'
        # ESCUADRADORA -> 'E'
        # CANTEO       -> 'C'
        maquina_map = {
        ...
        df["proceso_bloque"] = df["proceso_bloque"].astype(str).str.strip().str.upper()
        df["Maquina"] = df["proceso_bloque"].map(maquina_map)
        
        # 4. Mapeo inteligente del material (Tablero / Retazo)
        def determinar_material(row):
            proc = row["proceso_bloque"]
            if proc == "CANTEO":
                return "Canteo"
            val = str(row["tipo_tablero_retazo"]).strip().lower()
            if "retazo" in val:
                return "Retazo"
            elif "tablero" in val or "placa" in val:
                return "Tablero"
            return "Tablero"  # Valor por defecto si hay cantidad registrada

        df["Material"] = df.apply(determinar_material, axis=1)
        
        # 5. Normalización de cantidades
        df["Cantidad"] = pd.to_numeric(df["cantidad"], errors='coerce').fillna(0)
        
        # 6. Procesamiento seguro de fechas
        ...
        # Limpieza final de columnas requeridas
        df = df.dropna(subset=["Fecha_Corte", "Maquina"])
        return df[["Fecha_Corte", "Maquina", "Material", "Cantidad"]]
        
    except Exception as e:
        st.error(f"Error analítico en el procesamiento desde Supabase: {e}")
        return pd.DataFrame()
...
def mostrar():
    ...
    c_titulo.markdown("<h3 style='margin: 0px; padding: 0px; line-height: 1.2;'>📊 Rendimiento (Taller)</h3>", unsafe_allow_html=True)
    ...
    if c_sync.button("🔄 Actualizar", use_container_width=True, type="primary", help="Limpia la caché y descarga los datos nuevos de Supabase inmediatamente."):
        st.cache_data.clear()
    ...
    if df_raw.empty:
        st.info("📂 Esperando registros estructurados en las tablas de Supabase...")
        return
