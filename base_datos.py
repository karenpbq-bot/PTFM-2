...
# --- AGREGAR AL FINAL DE LA SECCIÓN 6 ---

def marcar_atencion_incidencia(id_incidencia, campo):
    """Registra la fecha actual en formato DD/MM/AAAA en el campo especificado."""
    try:
        supabase = conectar()
        fecha_str = datetime.now().strftime("%d/%m/%Y")
        supabase.table("incidencias").update({campo: fecha_str}).eq("id", id_incidencia).execute()
        return True
    except Exception as e:
        st.error(f"Error al marcar atención: {e}")
        return False

def guardar_obs_gestion(id_incidencia, observacion):
    """Guarda el comentario de gestión en la incidencia."""
    try:
        supabase = conectar()
        supabase.table("incidencias").update({"obs_gestion": observacion}).eq("id", id_incidencia).execute()
        return True
    except:
        return False

def obtener_datos_reporte_incidencias():
    """Obtiene una lista limpia para exportar a Excel/CSV."""
    try:
        supabase = conectar()
        res = supabase.table("incidencias").select("id, created_at, tipo_requerimiento, categoria, estado, fecha_almacen, fecha_recepcion, fecha_teowin, obs_gestion, proyectos(proyecto_text)").execute()
        if not res.data: return pd.DataFrame()
        
        # Aplanamos el nombre del proyecto
        for r in res.data:
            r['Proyecto'] = r['proyectos']['proyecto_text'] if r.get('proyectos') else "N/A"
            del r['proyectos']
        return pd.DataFrame(res.data)
    except:
        return pd.DataFrame()
