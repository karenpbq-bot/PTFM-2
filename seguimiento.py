...
def mostrar(supervisor_id=None):
    ...
    with st.expander("🔍 Selección de Proyecto", expanded=not st.session_state.get('id_p_sel')):
        ...
        bus_p = c1.text_input("Buscar proyecto...", key="bus_seg_MASTER_V2")
    ...
    # --- D. SECCIÓN DE CONFIGURACIÓN Y HERRAMIENTAS (RESTAURADA) ---
    ...
    with st.expander("⚙️ Herramientas y Gestión Masiva"):
        ...
        with t1:
            ...
            for i, h in enumerate(HITOS_LIST):
                pesos[h] = cols_w[i % 4].number_input(f"{h} (%)", value=float(pesos.get(h, 12.5)), step=0.5, key=f"pw_{h}")
        with t2:
            f1, f2 = st.columns(2)
    ...
    # --- H. RENDERIZADO (FLUYE SIN RECARGAS) ---
    cambios_df = st.data_editor(
        ...
        hide_index=True, use_container_width=True, key=f"mat_v_{id_p}_{st.session_state.ref_matriz}"
    )
    ...
    if btn_marcar:
        ...
        for idx, row in cambios_df.iterrows():
            ...
            for h_nom in HITOS_LIST:
                simb = MAPEO_HITOS[h_nom]
                if bool(row[simb]) and segs[(segs['producto_id'] == pid) & (segs['hito'] == h_nom)].empty:
                    nuevos.append({"pid": pid, "hito": h_nom})
        st.session_state.cambios_pendientes = nuevos
        st.rerun()

    if btn_db:
        if not st.session_state.cambios_pendientes: st.warning("No hay cambios rojos."); st.stop()
        lote = []
        f_hoy = datetime.now().strftime("%d/%m/%Y")
        for p in st.session_state.cambios_pendientes:
            match = segs[(segs['producto_id'] == p['pid']) & (segs['hito'] == p['hito'])]
            obs = match['observaciones'].iloc[0] if not match.empty else ""
            if p['hito'] == HITOS_LIST[0]: obs = cambios_df[cambios_df['id'] == p['pid']]['Observaciones'].iloc[0]
            lote.append({"producto_id": p['pid'], "hito": p['hito'], "fecha": f_hoy, "observaciones": obs, "supervisor_id": supervisor_id})
        ...
        if lote:
            ...
            st.session_state.cambios_pendientes = []; st.cache_data.clear(); st.session_state.ref_matriz += 1; st.rerun()
    ...
    def calc_v(df_m, df_s, pend):
        ...
        ids = df_m['id'].tolist()
        pts = sum([pesos.get(h, 0) for h in df_s[df_s['producto_id'].isin(ids)]['hito']])
        for p in pend:
